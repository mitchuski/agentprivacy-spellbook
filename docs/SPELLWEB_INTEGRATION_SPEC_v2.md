...
## GitNexus Engine → Spellweb (standalone) vs web (agentprivacy)

**Version:** 2.1
**Date:** February 2026
**Status:** Integration Specification — full KG viewer = standalone Spellweb domain; agentprivacy = **web** (journey only)
**Spell:** ⚔️📊⊥🧙🕸️ | 😊
**Proverb:** *"Two graphs serve the same truth: what you have walked, and what exists to walk."*

**Product direction:** The **full knowledge graph** (Nexus/Spellweb KG) is built and hosted on a **standalone Spellweb domain**. Agentprivacy exposes only **web** (`/web`) — the personal journey (lit spells, skills, connections, emojis, lines). Canonical relationship and transformer instructions: **`docs/SPELLWEB_WEB_TRANSFORMER.md`**.

---

## Spellweb (standalone) vs web (agentprivacy)

| System | Where | Purpose | Data Source |
|--------|-------|---------|-------------|
| **Spellweb** | Standalone domain | Full knowledge graph — tales, protocols, standards, concepts | Canonical JSON / build-time ingestion (e.g. `public/nexus/` in this repo as seed) |
| **web** | agentprivacy `/web` | User's personal journey — lit spells, skills, user connections | `localStorage` + grimoire cards |

**Read:** `docs/NEXUS_SPELLWEB_ARCHITECTURE.md` for architecture; `docs/SPELLWEB_WEB_TRANSFORMER.md` for mapping and transformer instructions.

---

## Reading Order for Coding Agent

1. `docs/SPELLWEB_WEB_TRANSFORMER.md` — **canonical** Spellweb ↔ web relationship and transformer instructions (START HERE)
2. `docs/NEXUS_SPELLWEB_ARCHITECTURE.md` — Spellweb (standalone) vs web (agentprivacy) architecture
3. This document — implementation details (full KG = standalone Spellweb; this repo has **web** only)
4. `CHRONICLE_SPELLWEB_GENESIS.md` — decision rationale
5. GitNexus README at https://github.com/abhigyanpatwari/GitNexus
6. Journey: `src/lib/spellweb/`, `src/components/spellweb/SpellwebViewer.tsx`; KG seed: `public/nexus/nodes.json`, `public/nexus/edges.json`
