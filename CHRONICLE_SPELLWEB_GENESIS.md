...
**Resolution (Feb 2026):** Spellweb as the *full* knowledge graph viewer is now scoped to a **standalone domain** (separate product). Agentprivacy keeps **web** (`/web`) as the personal journey only — lit spells, skills, connections, reflections (emojis, lines, constellation). See `docs/SPELLWEB_WEB_TRANSFORMER.md` for the canonical relationship and transformer instructions between Spellweb and web.
