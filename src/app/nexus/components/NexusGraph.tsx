...
import type { NexusNode, NexusEdge } from '@/lib/nexus/types';
import { GUILD_COLORS, MATURITY_SIZES } from '@/lib/nexus/types';

type SimNode = NexusNode & { x?: number; y?: number; vx?: number; vy?: number };
type SimLink = NexusEdge & { source: SimNode; target: SimNode };
...
export default function NexusGraph({ nodes, edges }: NexusGraphProps) {
  ...
  return (
    <div ref={containerRef} className="w-full h-full min-h-[520px] relative">
      <svg ref={svgRef} width={width} height={height} className="bg-transparent overflow-visible">
        <defs>
          <linearGradient id="nexus-gradient-amber-violet" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="#D4A017" />
            <stop offset="100%" stopColor="#7C6FEF" />
          </linearGradient>
          <marker id="nexus-arrow" viewBox="0 -5 10 10" refX="12" refY="0" markerWidth="6" markerHeight="6" orient="auto">
        ...
        <g className="zoom-group">
          <g className="edges">
            {edges.map((edge, i) => {
              ...
              const stroke = edge.type === 'guild_bridge' ? 'url(#nexus-gradient-amber-violet)' : '#6B7280';
              ...
              return (
                <line
                  ...
                  markerEnd={isPrincipleExtends ? 'url(#nexus-arrow)' : undefined}
